...
Deploying is not `az functionapp deploy` — that endpoint rejects this package (415 for a trivial zip, 502 for the real one). The workflows upload it to the deployment container as `released-package.zip`, call `syncfunctiontriggers`, then restart the app. Both deploy paths share `.github/actions/deploy-function-app` and `.github/actions/configure-ui-runtime-config`, so change the composite action rather than one workflow. In prod the API deploys and passes its health check *before* the UI, so a function app that never comes up leaves prod on the old release; `release-prod.yml` also takes a `workflow_dispatch` with a `tag` to redeploy an earlier release, which is the rollback path. Deploying the API and running the end-to-end tests is one job holding one concurrency group, because every PR deploys to the single dev function app and a group spanning two jobs would let the next run deploy between them.
