  ...
  <div>
    ...
    >
      <template v-slot:top>
        ...
        >
          ...
          >
            ...
            <DiaryEditor :title="editedItem?.title" :author="editedItem?.author" :description="editedItem?.description" @submit="onAddDiary" @close="close"></DiaryEditor>
          </v-dialog>
          <v-dialog v-model="dialogDelete" max-width="500px">
            <v-card>
              <v-card-title class="text-h5">Are you sure you want to delete this diary?</v-card-title>
              <v-card-actions>
                <v-spacer></v-spacer>
      ...
      <template v-slot:item.actions="{ item }">
        <v-btn v-if="state.isAuthenticated" icon @click="editItem(item)" size="small">
          <v-icon>
            mdi-pencil
          </v-icon>
        </v-btn>
        <v-btn v-if="state.isAuthenticated" icon @click="deleteItem(item)" size="small">
          <v-icon>
            mdi-delete
          </v-icon>
        </v-btn>
