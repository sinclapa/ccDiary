...
1. **Setup**: Run `setuplocal.ps1` for initial environment setup (Azure auth, Entra app, certs, .env files, user secrets)
...
- `.env.dev.local` - Local dev environment variables (created by setuplocal.ps1)
...
5. `setuplocal.ps1` handles all local setup: Azure auth, Entra app, certs, .env, user secrets
6. Docker Compose uses `.env` file in `src/api/` for secrets (generated by setuplocal.ps1)
