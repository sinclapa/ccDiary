...
ccDiary is a full-stack diary application — ASP.NET Core 8 API + Vue 3/Vuetify SPA, deployed to Azure (Functions Flex Consumption + Static Web Apps + Table/Blob Storage), authenticated with Microsoft Entra ID.
...
The API is an ordinary ASP.NET Core app, and nothing in `Program.cs` knows it is hosted by Azure Functions. Three files at the root of `ccDiaryApi` make it one:

- `host.json` declares a **custom handler** with `enableProxyingHttpRequest`, so the Functions host forwards raw HTTP rather than a binding payload, and sets `routePrefix: ""` so the app's own `api/v1/...` routes are unchanged.
- `proxy/function.json` is a single catch-all HTTP trigger (`route: "{*path}"`, anonymous) — the app still does its own authentication.
- `run.sh` execs the published binary with `--urls http://127.0.0.1:$FUNCTIONS_CUSTOMHANDLER_PORT`, the port the host assigns per instance.

Two consequences are not visible from the app's own code:
...
- **The CI identity has no password at all.** `ca-ccdiary-<env>-credentials` carries a federated credential (`github-actions-<env>`) trusting `https://token.actions.githubusercontent.com` for the subject `repo:sinclapa/ccDiary:environment:<env>`, and `azure/login` presents the token GitHub mints for the run instead of a stored secret. The jobs that sign in therefore need `id-token: write`, and `vars.AZURE_CLIENT_ID` / `vars.TENANT_ID` / `vars.AZURE_SUBSCRIPTION_ID` are identifiers, not credentials. There is no `AZURE_CREDENTIALS` secret and nothing to rotate.
- The subject pins the exchange to one repository **and one environment**, so the environments' deployment branch policies are what decide which refs can obtain it: `prod` allows `main` and `v*` tags, `staging` allows `main`, and `dev` is open because PR previews deploy from feature branches. Widening a policy widens who can reach that environment's Azure access.
- **Graph and API tokens are federated too.** `ccdiary-<env>` carries its own `github-actions-<env>` credential, and `.github/actions/entra-token` exchanges the run's OIDC token for an app-only token of whatever scope a step needs — Graph for the preview redirect steps, the API's own audience for seeding before the end-to-end run. The app's client secret still exists, but only in Key Vault, where `GraphService` reads it; CI never gets a copy.
- **Every PR runs in the `dev` environment**, so anything stored as a dev secret is reachable from any branch in the repo. Only `AZURE_STATIC_WEB_APPS_API_TOKEN` remains there. Adding a secret that a workflow does not read puts it within reach for nothing — the application's own configuration belongs in Key Vault.
